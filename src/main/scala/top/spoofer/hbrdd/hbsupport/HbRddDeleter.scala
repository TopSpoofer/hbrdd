 ...
 * @tparam A 类型参数
 */
final class RDDDelete[A](val rdd: RDD[(String, Map[String, Set[A]])], val deleter: DeleteTool[A]) extends HbRddDeleteCommon[A] with Serializable {
 ...
 * @tparam A 类型参数
 */
final class SingleFamilyRDDDelete[A](val rdd: RDD[(String, Set[A])], val deleter: DeleteTool[A]) extends HbRddDeleteCommon[A] with Serializable {
...
final class RowKeyRDDDelete(val rdd: RDD[String], val deleter: DeleteTool[String]) extends HbRddDeleteCommon[String] with Serializable {
  /**
   * 删除行数据
   * @param table 数据表
   * @param config 配置
